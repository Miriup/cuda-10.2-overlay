...
cuda_get_host_compiler() {
	...
	while ! nvcc -v -ccbin "${NVCC_CCBIN}" - -x cu <<<"int main(){}" &>> "${T}/cuda_get_host_compiler.log" ; do
		...
		while true; do
			...
			#NVCC_CCBIN="${compiler}-$(portageq metadata / installed ${package}-${package_version} SLOT)"
			if [ "${compiler_type}" == "gcc" ] && ver_test ${package_version/#<${package}-/} -lt 10; then
				NVCC_CCBIN="${compiler}-$(ver_cut 1-3 "${package_version/#<${package}-/}")"
			else
...
src_unpack() {
	git-r3_fetch https://github.com/${PN}/${PN} ${PV} ${P}
	#git-r3_fetch https://github.com/${PN}/${PN}_contrib ${ADE_PV} ade-${ADE_PV}
	if use contrib; then
		git-r3_fetch https://github.com/${PN}/${PN}_contrib ${PV} ${PN}_contrib-${PV}
		if use dnnsamples; then
			git-r3_fetch https://github.com/${PN}/${PN}_3rdparty ${QRCODE_COMMIT} ${PN}_3rdparty-${QRCODE_COMMIT} ${PN}_3rdparty-${QRCODE_COMMIT}
			git-r3_fetch https://github.com/${PN}/${PN}_3rdparty ${DNN_SAMPLES_FACE_DETECTOR_COMMIT} ${PN}_3rdparty-${DNN_SAMPLES_FACE_DETECTOR_COMMIT}
		fi
		if use contribxfeatures2d; then
			git-r3_fetch https://github.com/${PN}/${PN}_3rdparty ${XFEATURES2D_BOOSTDESC_COMMIT} ${PN}_3rdparty-${XFEATURES2D_BOOSTDESC_COMMIT}
			git-r3_fetch https://github.com/${PN}/${PN}_3rdparty ${XFEATURES2D_VGG_COMMIT} ${PN}_3rdparty-${XFEATURES2D_VGG_COMMIT}
		fi
		if use contribdnn; then
			git-r3_fetch https://github.com/${PN}/${PN}_3rdparty ${FACE_ALIGNMENT_COMMIT} ${PN}_3rdparty-${FACE_ALIGNMENT_COMMIT}
		fi
		#if use cuda; then
		#	git-r3_fetch https://github.com/NVIDIA/NVIDIAOpticalFlowSDK ${NVIDIA_OPTICAL_FLOW_COMMIT} NVIDIAOpticalFlowSDK-${NVIDIA_OPTICAL_FLOW_COMMIT}
		#fi
		#git-r3_fetch https://github.com/${PN}/${PN}_contrib 667a66ee0e99f3f3263c1ef2de1b90d9244b7bd4 ${PN}_contrib-4.10.0-3607
	fi
	# checkout
	git-r3_checkout https://github.com/${PN}/${PN} ${P} ${P}
	#git-r3_checkout https://github.com/${PN}/${PN}_contrib ade-${ADE_PV} ade-${ADE_PV}
	if use contrib; then
		git-r3_checkout https://github.com/${PN}/${PN}_contrib ${PN}_contrib-${PV} ${PN}_contrib-${PV}
		if use dnnsamples; then
			git-r3_checkout https://github.com/${PN}/${PN}_3rdparty ${PN}_3rdparty-${QRCODE_COMMIT} ${PN}_3rdparty-${QRCODE_COMMIT}
			git-r3_checkout https://github.com/${PN}/${PN}_3rdparty ${PN}_3rdparty-${DNN_SAMPLES_FACE_DETECTOR_COMMIT} ${PN}_3rdparty-${DNN_SAMPLES_FACE_DETECTOR_COMMIT}
		fi
		if use contribxfeatures2d; then
			git-r3_checkout https://github.com/${PN}/${PN}_3rdparty ${PN}_3rdparty-${XFEATURES2D_BOOSTDESC_COMMIT} ${PN}_3rdparty-${XFEATURES2D_BOOSTDESC_COMMIT}
			git-r3_checkout https://github.com/${PN}/${PN}_3rdparty ${PN}_3rdparty-${XFEATURES2D_VGG_COMMIT} ${PN}_3rdparty-${XFEATURES2D_VGG_COMMIT}
		fi
		if use contribdnn; then
			git-r3_checkout https://github.com/${PN}/${PN}_3rdparty ${PN}_3rdparty-${FACE_ALIGNMENT_COMMIT} ${PN}_3rdparty-${FACE_ALIGNMENT_COMMIT}
		fi
		#if use cuda; then
		#	git-r3_checkout https://github.com/NVIDIA/NVIDIAOpticalFlowSDK NVIDIAOpticalFlowSDK-${NVIDIA_OPTICAL_FLOW_COMMIT} NVIDIAOpticalFlowSDK-${NVIDIA_OPTICAL_FLOW_COMMIT}
		#fi
		#git-r3_checkout https://github.com/${PN}/${PN}_contrib ${PN}_contrib-4.10.0-3607
	fi
}
...
src_prepare() {
	...
	if use contrib; then
		...
		if type -P nvcc &> /dev/null && ver_test "$(nvcc --version | tail -n 1 | sed -n 's/.*release \([0-9]\{1,\}\.[0-9]\{1,\}\),.*/\1/p')" -ge 12.4; then
